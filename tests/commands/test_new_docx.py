"""Unit tests for new-docx command."""
...
class TestCmdNewDocx(unittest.TestCase):
    """Tests for cmd_new_docx function."""
    ...
    def test_new_docx_no_args(self) -> None:
        """Test new-docx command with no arguments."""
        sys.stdout = StringIO()
        sys.stderr = StringIO()
        ...
        self.assertIn("Usage: q4s new-docx <directory>", stderr_output)

    def test_new_docx_invalid_directory_name(self) -> None:
        """Test new-docx command with invalid directory name."""
        sys.stdout = StringIO()
        sys.stderr = StringIO()

        result = cmd_new_docx(["-invalid"])
        stderr_output = sys.stderr.getvalue()

        self.assertEqual(result, 1)
        self.assertIn("Error: Invalid directory name", stderr_output)

    def test_new_docx_creates_directory_and_files(self) -> None:
        """Test new-docx command creates directory, qmd file, and symlink."""
        sys.stdout = StringIO()
        sys.stderr = StringIO()

        result = cmd_new_docx(["test-document"])
    ...
    def test_new_docx_directory_already_exists(self) -> None:
        """Test new-docx command when directory already exists but qmd doesn't."""
        ...
        result = cmd_new_docx(["existing-dir"])
        stdout_output = sys.stdout.getvalue()
    ...
    def test_new_docx_qmd_file_already_exists(self) -> None:
        """Test new-docx command when qmd file already exists."""
        ...
        result = cmd_new_docx(["existing-document"])
        stderr_output = sys.stderr.getvalue()
    ...
    def test_new_docx_with_subdirectory_path(self) -> None:
        """Test new-docx command with subdirectory in name."""
        sys.stdout = StringIO()
        sys.stderr = StringIO()

        result = cmd_new_docx(["subdir/my-document"])
        stdout_output = sys.stdout.getvalue()
...
class TestCmdNewDocxErrorCases(unittest.TestCase):
    """Tests for cmd_new_docx error handling."""
    ...
    def test_directory_creation_error(self) -> None:
        ...
        result = cmd_new_docx(["blocked/subdir"])
        stderr_output = sys.stderr.getvalue()
    ...
    def test_qmd_file_write_error(self) -> None:
        ...
        try:
            result = cmd_new_docx(["readonly-dir"])
            stderr_output = sys.stderr.getvalue()
